...
| import_baird_activity.py | Phase 4 — **NEW 2026-09-16.** Baird Activity CSV import → `dbo.baird_activity` (buys/sells, fees, dividends/interest/capital gains) — a different export than Holdings (transaction-level, not point-in-time positions). Pure review/visibility for Tom, not wired into Budget vs Actual. Auto-detects two column layouts Baird has used across export vintages, plus a plain-signed vs accounting-style Amount/Price format difference between them; `activity_id` hashes parsed values + an occurrence counter (never raw text) for idempotent re-import of Tom's normal overlapping monthly export window. Reuses `import_baird_holdings.py`'s account-name normalization and currency parser. | ✅ Live — 673 rows (2026-01-01 to present), verified idempotent |
| principal_sync.py | Phase 4 — Pulls Principal/Baird 401k holdings via Plaid Investments (/investments/holdings/get), upserts dbo.accounts/dbo.securities/dbo.holdings. Created 2026-08-01. | ✅ 2026-09-14 (ISSUE-009 closed, AFAS 0cee6b6): wired into the pipeline, deployed to Finance-ingest-Tom-v6 and verified via Application Insights (real HTTP-triggered run) — now run via `http_monthly_ingest_all` as of Session 22 rather than the deregistered `monthly_sync.py` timer. Also captures Plaid's `sector`/`industry` per security into `dbo.securities` (AFAS 64e45cc) — **confirmed live in production 2026-09-16**: 10 of 11 current 401k holdings carry real sector/industry data with `updated_at` timestamped to a real `http_monthly_ingest_all` run. Confirmed via a direct raw-API check that Plaid returns `cost_basis: null` and empty `tax_lots` for all 11 holdings — genuinely unavailable from this institution, not a pipeline gap. |
...
| Budget vs Actual | ✅ Complete (2026-09-17) — 12-month matrix (Year/Month rows, Expense\|Income column groups, Amount/Budget/A-B), a current-month matrix (Type/Category rows, same 3 values, with data bars), and a bar chart (Actual-minus-Budget, Expense only, last 12 months). Built on 3 new measures — `Signed Actual`/`Signed Budget`/`Signed Variance (A-B)` — that flip sign for Expense only, since `actual_amount`/`budget_amount` are stored as positive magnitudes for both types. Surfaced a real budget_targets gap — Sep-Dec 2025 had actuals but no budget line at all, fixed by backfilling from 2026's same months (script 116) — and ISSUE-046 (script 119, 2026-09-21): `vw_budget_vs_actual`'s `actual` CTE summed `ABS(amount)` per row before aggregating, so a refund/return (positive-signed row within an Expense category) added to spend instead of netting against the original purchase; fixed via `ABS(SUM(amount))` instead. Income was never affected (rows are already virtually all positive-signed). |
...
#### vw_baird_activity (added 2026-09-16, scripts 111-113)
Categorizes `dbo.baird_activity` (raw Baird Activity CSV import — see `import_baird_activity.py`) for Tom's review needs, independent of any budget logic. `activity_category` buckets Baird's raw `activity_type` values into `Trade` / `Fee` / `Income` / `Cash Movement` / `Other` (anything unmapped falls to `Other` rather than silently vanishing — a genuinely new Baird activity type stays visible). `trade_direction` (`Buy`/`Sell`, Trade rows only) reads off the sign of `quantity`. `is_reinvestment` flags `Asset Bought-reinv` (DRIP) rows specifically, since they fire on every dividend across every fund position and would otherwise swamp genuine discretionary buy/sell activity in a "recent trades" view — `Asset Bought` (a real discretionary purchase, added script 113) stays `Trade`/`Buy` but `is_reinvestment = 0`. Kept as a view rather than baked into the import so categorization can be refined without re-importing, same pattern as `net_worth_category` over `vw_holdings_all`.
